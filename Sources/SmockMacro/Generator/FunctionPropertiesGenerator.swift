...
enum FunctionPropertiesGenerator {
    static func expectationsOptionsClassDeclaration(
      ...
      return try ClassDeclSyntax(
        ...
        genericParameterClause: genericParameterClauseElements.count > 0 ? ": \(raw: genericParameterClauseElements.joined(separator: ", ")) " : nil,
        memberBlockBuilder: {
  ...
  static func expectedResponseVariableDeclaration(
    ...
    staticName: Bool
      ...
      let expectedResponseType = "\(variablePrefix.capitalizingComponentsFirstLetter())_ExpectedResponse"
      let variablePrefix = VariablePrefixGenerator.text(for: functionDeclaration)
      let parameterList = functionDeclaration.signature.parameterClause.parameters
        let inputMatcherType = parameterList.count > 0 ? "\(variablePrefix.capitalizingComponentsFirstLetter())_InputMatcher" : "AlwaysMatcher"
  ...
  static func allVerificationsDeclaration(functionDeclarations: [FunctionDeclSyntax]) throws
    -> StructDeclSyntax
  {
