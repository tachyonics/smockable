    ...
    private static func determineTypeConformance(
        ...
        while let first = remainingInput.first {
            ...
            if first == ">" || isEndToken {
                ...
                if let lastElement = stack.popLast() {
                    ...
                    if !currentToken.isEmpty {
                        typeConformance = getConformance(currentType: currentToken)
                    ...
                    case .confirmedConformance(let possibleKeyConformance):
                        if let secondLastElement = stack.last, secondLastElement.isDictionaryStartToken(canBeLiteralToken: isEndToken) {
                            _ = stack.popLast()
                            let collectionConformance = getDictionaryConformance(keyConformance: possibleKeyConformance, valueConformance: typeConformance)
                            stack.append(.confirmedConformance(collectionConformance))
                        }
            ...
            // if the element is a dictionary seperator
                ...
                if let secondLastElement = stack.last, secondLastElement.isDictionaryStartToken(canBeLiteralToken: isSeperatorToken) {
                    if !currentToken.isEmpty {
